...
class HomeController extends Controller {
	...
	public function __construct() {
		...
		//$api_url = 'http://125.213.130.104:8080';
		$api_url = 'http://10.10.20.3:3001';
		$api_url2 = 'http://localhost:8006';

		$this->jar = new \GuzzleHttp\Cookie\CookieJar();
		$this->api_path = 'bank/';
        $this->curl = new Client(['base_uri' => $api_url,'timeout'  => 60.0,'cookies'=>Session::get('cookieJar'),'headers' => ['Accept' => 'application/json']]);
	...
	public function dashboard()
	{
		...
		try{
		$res_tree = $this->curl->request('POST', $this->api_path.'getProfiles'.'?locale=en', [
	...
	public function postLogin(Request $request){
        ...
        //$res = curl('login','{"loginId":"'.$login_item['loginId'].'", "passwd":"'.$login_item['passwd'].'"}');
		try{
		$res = $this->curl->request('POST', $this->api_path.'login'.'?locale=en', [
				'headers' => ['Content-Type' => 'application/json', 'Accept' => 'application/json'],
    ...
    public function fetchDataTable(Request $request){
		...
		try {
			$res = $this->curl->request('POST', $this->api_path . $menu . '/' . $url_action.'?locale=en', [
					'headers' => ['Content-Type' => 'application/json', 'Accept' => 'application/json'],
					'body' => $json,
					'timeout' => 15.0,
					'cookies' => Session::get('cookieJar'),
	...
	public function postDetail2(Request $request){
		...
		$json = json_encode($data);
		try{
		$res = $this->curl2->request('POST', $this->api_path.$menu.'/'.$url_action.'?locale=en', [
				'headers' => ['Content-Type' => 'application/json', 'Accept' => 'application/json'],
				'body'    => $json,
				'timeout' => 15.0,
				'cookies'=>Session::get('cookieJar'),
	...
	public static function getTrxStatusDetailView(Request $request){
		...
		$data['service'] = $request->input('service');
		$data['createdByUserCode'] = $request->input('createdByUserCode');
                $data['menuCodeExecution'] = $request->input('menuCodeExecution');
...
if($request->input('fileName')){
	...
	public function downloadTrade(Request $request){
...
if($request->input('fileName')){
	...
	public function uploadFile(Request $request){
		...
		try{
		$res = $this->curl->request('POST', $this->api_path.'MNU_GPCASH_MT_HELPDESK/upload'.'?locale=en', [
				'multipart' => $multipart,
				'body'    => $json,
		...
		if(isset($msg->uploadDateTime)){
			$result['uploadDateTime'] = $msg->uploadDateTime;
		}
	...
	public function uploadDoc(Request $request){
		...
		$json = json_encode($data);
		$content = file_get_contents($request->file);
		// $arrFile = str_split($request->file);
		// $data['file'] = $arrFile;
		...
		try{
		$res = $this->curl->request('POST', $this->api_path.'MNU_GPCASH_BANK_TF_EXPORT/uploadDocument'.'?locale=en', [
	...
	public function uploadTrade(Request $request){
		$data = array();
		...
		$json = json_encode($data);
		$content = file_get_contents($request->file,false,$context);
		...
		try{
		$res = $this->curl->request('POST', $this->api_path.$menu.'/uploadDocument'.'?locale=en', [
				// 'headers' => ['Content-Type' => 'multipart/form-data'],
				// 'headers' => ['Content-Type' => 'application/json', 'Accept' => 'application/json'],
				'multipart' => $multipart,
				'body' => $json,
    ...
    public function uploadDocImportBill(Request $request){
		$data = array();
		...
		try{
		$res = $this->curl->request('POST', $this->api_path.'MNU_GPCASH_BANK_TF_IMPORT_BILL/uploadDocument'.'?locale=en', [
				// 'headers' => ['Content-Type' => 'multipart/form-data'],
				// 'headers' => ['Content-Type' => 'application/json', 'Accept' => 'application/json'],
				'multipart' => $multipart,
				'body' => $json,
