	...
	<script>
		...
		$(document).ready(function(){
		      ...
		      methods: {
		        setView:function(view,action){
		        	...
		        	if(view=='dashboard'){
		        		...
		        		url = viewURL;
                        //filter reset if menu changes
                        var newMenu = $('#'+view).data('menu');
                        var olddMenu = viewData.menu;
                        if(newMenu!=olddMenu){
                            window.searchParam = null;
                        }
		        		viewData.service = $('#'+view).data('service');
		        		//viewData.parent = $('#'+view).data('parent-menu');
		        		viewData.menu = newMenu;
		        		if(!action){
		        			action='landing';
		        		}
			...
			$(document).ajaxStart(function() {
				...
				//MISMATCH _TOKEN
				if(jqxhr.status==498){
					window.location = '{{ route("login") }}?error=Session Expired.';
