                ...
                <div class="box-body">
                    ...
                    <div class="box-body">
                                ...
                                <table id="list" class="table table-bordered table-striped dataTable" border="2" cellpadding="2"
                                 ...
                                 <table id="list_detail" class="table table-bordered table-striped dataTable" border="2" cellpadding="2"
...
<script>
    ...
    $(document).ready(function () {
        ...
        oTable = $('#list').DataTable({
         ...
         oTable_detail = $('#list_detail').DataTable({
            "paging": false,
            "ordering": false,
            "info": false,
            "destroy": true,
        ...
        $('#confirm').on('click', function () {
            ...
            var $codeField = $('#code');
            var $codeFormGroup = $codeField.closest('.row');
            var $codeHelpBlock = $codeFormGroup.find('.help-block');

            if($codeField.val() === '') {
                hasEmptyField = true;
                $codeFormGroup.addClass('has-error');
                $codeField.css('border-color', '#a94442');
                $codeHelpBlock.html('<ul class="list-unstyled"><li style="color:#a94442;">This field is required.</li></ul>').show();
            } else {
                $codeFormGroup.removeClass('has-error');
                $codeField.css('border-color', '');
                $codeHelpBlock.html('').hide();
            }

            // Validate Limit Setup Name
            var $nameField = $('#name');
            var $nameFormGroup = $nameField.closest('.row');
            var $nameHelpBlock = $nameFormGroup.find('.help-block');

            if($nameField.val() === '') {
                hasEmptyField = true;
                $nameFormGroup.addClass('has-error');
                $nameField.css('border-color', '#a94442');
                $nameHelpBlock.html('<ul class="list-unstyled"><li style="color:#a94442;">This field is required.</li></ul>').show();
            } else {
                $nameFormGroup.removeClass('has-error');
                $nameField.css('border-color', '');
                $nameHelpBlock.html('').hide();
            }
 	...
 	$('#code').alphanum({
                ...
                success: function (data) {
                    ...
                    if (result.status=="200") {
                            ...
                            $.each(result.result, function (idx, obj) {
                                ...
                                if(lastServiceName==obj.serviceName){
                                    $('#matrix_id[value="'+obj.serviceCurrencyMatrixId+'"]').parent().prev().prev().children().css('opacity','0')
                                }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (result.status=="200") {
                       ...
                       currencyOption += '<option value="IDR">IDR</option>';
        ...
        function getTableData() {
            ...
            $("#list").find("tbody tr").each(function () {
                ...
                var obj = {}
                 if ($('#type').val() == 'edit') {
                     var dataLimit = JSON.parse($('#temp').attr('data-limit'));
                var dataVo = dataLimit.find(val=>val.serviceCode+val.serviceCurrencyMatrixId==service_code+matrix_id)

                    if (check == 1 || service_code == service_codeTemp) {
                        var serviceName = '';

                        if(lastServiceName!==dataVo.serviceName){
                            serviceName = dataVo.serviceName;
                        }

                       oTable_detail.row.add([
                                '<span id="service_name">'+serviceName +'</span>'+'<input id="service_code" name="service_code" class="form-control state_edit" value="' + obj.service_code + '" type="hidden">',
                                '<span id="currency_matrix_name">'+currency_matrix_name +'</span>'+'<input id="matrix_id" name="matrix_id" class="form-control state_edit" value="' + matrix_id + '" type="hidden">',
                                '<input id="num_trans" name="num_trans" class="form-control state_edit num_trans" value="'+num_trans+'" type="text" style="width:100%;text-align: right;"><span id="num_trans_view" class="state_view" style="float: right;">'+num_trans +'</span>',
                                curr_code,
                                '<input id="max_trans" name="max_trans" class="form-control state_edit max_trans numeric" value="'+max_trans+'" type="text" style="width:100%;text-align: right;"><span id="max_trans_view" class="state_view" style="float: right;">'+currencyFormat(max_trans)+'</span>'
                            ]).draw(false);

                       service_codeTemp = service_code;
                        lastServiceName = dataVo.serviceName;
                    }
