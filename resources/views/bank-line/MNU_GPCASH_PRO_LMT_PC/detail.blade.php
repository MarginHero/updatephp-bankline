...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#delete').on('click', function () {
           ...
           // $('.state_delete').show();
            var limit = $('#name').text();
            var content ='{{trans('form.confirm_delete_limit',['limit'=>'$limit'])}}';
            var content2 = content.replace('$limit',limit);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
                    ...
                    $('#temp').attr('data-limit',JSON.stringify(detail));
                    ...
                    if(detail){
                        ...
                        var groupedData = Object.values(detail.reduce((acc, current) => {
                           if (!acc[current.serviceCurrencyMatrixId]) {
                               acc[current.serviceCurrencyMatrixId] = {
                                   maxTrxAmountPerDay: current.maxTrxAmountPerDay,
                                   maxTrxPerDay: current.maxTrxPerDay,
                                   serviceCode: current.serviceCode,
                                   serviceName: current.serviceName,
                                   serviceCurrencyMatrixId: current.serviceCurrencyMatrixId,
                                   currencyMatrixName: current.currencyMatrixName,
                                   currencyCode: current.currencyCode,
                                items: []
                               };
                           }
                           acc[current.serviceCurrencyMatrixId].items.push(current);

                           return acc;
                       }, {}));

                       var sortedData = groupedData.sort((a, b) => {
