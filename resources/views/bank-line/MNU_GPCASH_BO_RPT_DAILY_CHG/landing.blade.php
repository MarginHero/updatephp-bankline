                        ...
                        <div class="container-fluid">
                            <div class="row">
                                <div class="mb-3 row form-group">
                                    ...
                                    <div class="col-md-6 row-date">
                                        <div class="row g-2 align-items-center">
                                            <div class="col-auto">
                                                <div class="input-group">
                                                    <span class="input-group-text"><i class="fa fa-calendar"></i></span>
                                                    <input type="text" id="dateFrom" name="dateFrom" class="form-control datepicker detail numeric shouldValidate" autocomplete="off" value="">
                                                </div>
                                                <div class="help-block with-errors"></div>
                                            </div>
                                            <div class="col-auto mx-2">
                                                <label class="col-form-label" style="padding-right: 11px!important;padding-left: 11px!important"><strong>to</strong></label>
                                            </div>
                                            <div class="col-auto">
                                                <div class="input-group">
                                                    <span class="input-group-text"><i class="fa fa-calendar"></i></span>
                                                    <input type="text" id="dateTo" name="dateTo" class="form-control datepicker numeric shouldValidate" autocomplete="off" value="">
                                                </div>
                                                <div class="help-block with-errors"></div>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.datepicker').datepicker({
            ...
            locale: 'id',
            autoclose:true,
            endDate: new Date() // Sets the maximum selectable date to today
        });
        ...
        $('#dateFrom').val(moment(new Date()).subtract(1, 'day').format("DD/MM/YYYY"));
        ...
        $('#dateFrom, #dateTo').on('change', function() {
            if($(this).val() != '') {
                ...
                $(this).siblings('.help-block.with-errors').html('');
            }
        });
        ...
        $('#organization').on('change', function() {
            // TODO - find company based on organization
            var branchId = $(this).val();
            getCorporate(branchId);
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
                    unitOption = '<option value="%" selected>All Service</option>';
    ...
    function getBranch() {
        ...
        $.ajax({
            ...
            success: function (data) {
                var result = JSON.parse(data);
                if (result.status=="200") {
                    unitOption = '<option value="%" selected>All Organization Unit</option>';
                    $.each(result.result, function (idx, obj) {
    ...
    function getCorporate(branchId) {
        var value = {
            branchId: branchId ?? "",
        };
        ...
        $.ajax({
            ...
            success: function (data) {
                var result = JSON.parse(data);
                if (result.status=="200") {
                    unitOption = '<option value="%" selected>All Company</option>';
                    $.each(result.result, function (idx, obj) {
    ...
    function requestDownload(fileFormat) {
        ...
        $('.shouldValidate').validator('validate');

        // Clear previous errors
        $('#dateFrom, #dateTo').removeClass('error');
        $('#dateFrom').siblings('.help-block.with-errors').html('');
        $('#dateTo').siblings('.help-block.with-errors').html('');
        ...
        if ($('#dateFrom').val() == '') {
            ...
            $('#dateFrom').siblings('.help-block.with-errors').html('<span style="color: red;">Please select From Date</span>');
            return;
        }
        if ($('#dateTo').val() == '') {
            $('#dateTo').addClass('error');
            $('#dateTo').siblings('.help-block.with-errors').html('<span style="color: red;">Please select To Date</span>');
            return;
        }
        if (checkDate() > 0) return;

        value.fromDate = $('#dateFrom').val();
        value.toDate = $('#dateTo').val();

        value.corporateId = $("#corporate").val();
        value.userId = $("#activityBy").val();
        value.branchId = $("#branch").val();
        value.serviceId = $("#service").val();
    ...
    function checkDate(){
        ...
        var today = moment(new Date(),"DD/MM/YYYY hh:mm").format("YYYY/MM/DD");
        if(date2 == '')
            date2 = today
        ...
        if (date2 > today) {
            var content ='{{trans('form.to_date_larger_than_today')}}';
            $.alert({
                title: '{{trans('form.warning')}}',
                content: content
            });
