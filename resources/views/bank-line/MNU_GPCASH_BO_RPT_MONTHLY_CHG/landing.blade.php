...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#searchMonth').on('change', function() {
            if($(this).val() != '' && $(this).val() != 'Select Month') {
    ...
    function getMonthDropList() {

        var url_action = 'monthDropList';
        var action = 'SEARCH';
        var menu = service;
        $.ajax({
            url: 'getAPIData',
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
                    ...
                    $.each(result.months, function (idx, obj) {
                        month += '<option value="' + obj.code +'">' + obj.value + '</option>';
                    });
                    $('#searchMonth').html(month);
                    $('#searchMonth').select2();
                } else {
                    flash('warning', result.message);
                }
	...
	function requestDownload(fileFormat){
        ...
        $('.row-month').validator('validate');

        // Clear previous errors
        $('#searchMonth').removeClass('error');
        $('#searchMonth').closest('.mb-3').find('.help-block.with-errors').html('');

        if ($('#searchMonth').val() == '' || $('#searchMonth').val() == 'Select Month') {
            $('#searchMonth').addClass('error');
            $('#searchMonth').closest('.mb-3').find('.help-block.with-errors').html('<span style="color: red;">Please Select Month</span>');
            return;
        }

        var validate = true;

        value.month = $('#searchMonth').val();
        value.serviceId = $('#service').val();
        value.branchId = $('#branch').val();
        value.corporateId = $('#searchCompany').val();
