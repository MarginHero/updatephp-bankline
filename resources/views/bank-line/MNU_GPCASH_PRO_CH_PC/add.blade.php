...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#confirm').on('click', function () {
            ...
            if($('#form-area').validator('validate').has('.has-error').length!=0 || hasError){
                return;
            }
        ...
        function getTableData() {
            ...
            $("#list").find("tbody tr").each(function () {
                ...
                var obj = {}
                if ($('#type').val() == 'edit') {
                    var dataCh = JSON.parse($('#temp').attr('data-limit'));
                    var dataVo = dataCh.find(val=>val.serviceCode+val.serviceChargeId==service_code+service_charge_id)
                    if (check == 1) {
                        oTable_detail.row.add([
                            '<input id="check" class="dt-checkboxes state_edit" value="" type="checkbox">',
                            '<span id="service_name">' + dataVo.serviceName + '</span>' + '<input id="service_code" name="service_code" class="form-control state_edit" value="' + service_code + '" type="hidden">',
                            '<span id="service_charge_name">' + service_charge_name + '</span>' + '<input id="service_charge_id" name="service_charge_id" class="form-control state_edit" value="' + service_charge_id + '" type="hidden">',
                            curr_code ,
                            '<input id="value" name="value" class="form-control state_edit value" value="'+value+'" type="text" style="width:100%;text-align: right;"><span id="value_view" class="state_view" style="float: right;">'+currencyFormat(value)+'</span>'
                        ]).draw(false);
                    }
