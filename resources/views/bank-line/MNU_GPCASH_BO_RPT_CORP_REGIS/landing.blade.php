...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#corporate').on('change', function() {
            if($(this).val() != '' && $(this).val() != 'Select Corporate') {
    ...
    function requestDownload(fileFormat) {
        ...
        if (searchByCorp) {
            ...
            if ($('#corporate').val() == '' || $('#corporate').val() == 'Select Corporate') {
                validate = false;
                $('#corporate').addClass('error');
                $('#corporate').closest('.mb-3').find('.help-block.with-errors').html('<span style="color: red;">Please Select Corporate</span>');
                return;
            }
