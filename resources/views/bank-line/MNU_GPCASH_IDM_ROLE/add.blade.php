...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#confirm').on('click', function () {
            ...
            var $dscpField = $('#dscp');
            var $dscpFormGroup = $dscpField.closest('.row');
            var $dscpHelpBlock = $dscpFormGroup.find('.help-block');

            if($dscpField.val() === '') {
                hasEmptyField = true;
                $dscpFormGroup.addClass('has-error');
                $dscpField.css('border-color', '#a94442');
                $dscpHelpBlock.html('<ul class="list-unstyled"><li style="color:#a94442;">This field is required.</li></ul>').show();
            } else {
                $dscpFormGroup.removeClass('has-error');
                $dscpField.css('border-color', '');
                $dscpHelpBlock.html('').hide();
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (result.status=="200") {
                        ...
                        $('#code').val(result.result[index].code).prop('disabled', true);
