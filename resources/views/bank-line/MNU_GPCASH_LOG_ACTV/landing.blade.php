	   ...
	   //      	value['fromDateVal'] = moment($('#fromDate').val()).format("DD/MM/YYYY HH:mm:ss");
         ...
         $('#list tbody').on('click', 'a', function (e) {
			...
			if (pendingTaskId !== undefined) {
                ...
                if(res=='done'){
                    ...
                    if(activityType!='UPDATE'&&activityType!='UPDATE_TERM'){
                        getMenuDetail(menuCode);
                    }else{
                        getMenuDetailViewUpdate(menuCode);
                    }
		...
		if (window.searchParam) {
            if (window.searchParam.searchby == '1') {
                ...
                $('#activityBy').val(window.searchParam.actionBy).change();
                $('#menuDroplist').val(window.searchParam.activityLogMenuCode).change();
						...
						},
						{
							...
							render: function ( data, type, full, meta ) {
								if(data.pendingTaskId != ''){
									return '<a href="javascript:void(0)" data-code="'+data.pendingTaskId+'" data-menuCode="'+data.menuCode+'" data-menuName="'+data.menuName+'" data-activityBy ="'+data.actionBy +' - '+ data.actionByUserName +'" data-activityDate = "'+ data.actionDate +'" data-referenceNo="'+data.referenceNo+'" data-description="'+data.uniqueKeyDisplay+'" data-activityType="'+data.actionType+'" data-status = "'+data.status+'" data-rjct = "'+data.rejectReason+'">'+data.referenceNo+'</a>';
								}else{
        ...
        $.ajax({
            ...
            success: function (data) {
                var result = JSON.parse(data);
                    unitOption = '<option value="" selected>All Menu</option>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
					unitOption = '<option value="" selected>All User</option>';
                    $.each(result.result, function (idx, obj) {
