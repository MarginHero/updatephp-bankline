                ...
                var result = JSON.parse(data);
                 if (result.status=="200") {
                   var detail = result.details;
                     ...
                     if(detail.internationalBankOrganizationName !== "")
                        $('#old_organization').text(detail.internationalBankOrganizationName);
                     ...
                     if(detail.department !== "")
                        $('#old_department').text(detail.department);
                     
                     if(detail.subDepartment !== "")
                        $('#old_subDepartment').text(detail.subDepartment);
                     
                     if(detail.streetName !== "")
                        $('#old_streetName').text(detail.streetName);
                     
                     if(detail.buildingNumber !== "")
                        $('#old_buildingNumber').text(detail.buildingNumber);
                     
                     if(detail.buildingName !== "")
                        $('#old_buildingName').text(detail.buildingName);
                     
                     if(detail.floor !== "")
                        $('#old_floor').text(detail.floor);
                     
                     if(detail.postBox !== "")
                        $('#old_postBox').text(detail.postBox);
                     
                     if(detail.postCode !== "")
                        $('#old_postCode').text(detail.postCode);
                     
                     if(detail.room !== "")
                        $('#old_room').text(detail.room);
                     
                     if(detail.townName !== "")
                        $('#old_townName').text(detail.townName);
                     
                     if(detail.townLocationName !== "")
                        $('#old_townLocationName').text(detail.townLocationName);
                     
                     if(detail.districtName !== "")
                        $('#old_districtName').text(detail.districtName);
                     
                     if(detail.countrySubDivision !== "")
                        $('#old_countrySubDivision').text(detail.countrySubDivision);
                     
                     if(detail.countryName !== "")
                        $('#old_country').text(detail.countryName);
                } else {
