            ...
            success: function (data) {
                ...
                var result = JSON.parse(data);
                 if (result.status=="200") {
                    ...
                    //old vlaue
                     var detail = result.oldValue.details.roleCodeList;
                     $('#old_code_1').text(result.oldValue.details.code);
                     $('#old_name').text(result.oldValue.details.name);
                     $('#old_unit').text(result.oldValue.details.branchCode + ' - ' + result.oldValue.details.branchName);
                     $('#old_email').text(result.oldValue.details.email);
                     $('#old_activeFrom').text(moment(result.oldValue.details.activeFrom,"DD/MM/YYYY hh:mm").format("DD/MM/YYYY"));
                     if(result.oldValue.details.activeTo) {
                         $('#old_activeTo').text(moment(result.oldValue.details.activeTo, "DD/MM/YYYY hh:mm").format("DD/MM/YYYY"));
                     }
                     $('#old_isPwdNeverExpired').text(result.oldValue.details.isPwdNeverExpired);
                     $('#old_status').text(result.oldValue.details.status);
                     $('#status').text(result.oldValue.details.status);
                     var status = $('#old_status').text();
                     if(status=="RESET"){
                         $('#reset').prop('disabled',false);
                         $('#unlock').prop('disabled',true);
                         $('#inactivate').prop('disabled',false);
