                ...
                <form class="form-horizontal" id="form-area">
                    ...
                    <div class="box-body">
                        <div class="container-fluid">
                            <div class="row">
                                <div class="mb-3 row">
                                    <label class="col-md-2 col-form-label text-end"><strong>Menu</strong></label>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#searchMenu').on('change', function() {
            if($(this).val() != '' && $(this).val() != 'Select Menu') {
    ...
    function getFormatDroplist() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var msg = '{{trans('form.conn_error')}}';
                flash('warning', msg);
                console.log('API Error:', xhr.status + " ," + " " + ajaxOptions + ", " + thrownError);
            }, complete: function (data) {
    ...
    function requestDownload(fileFormat){
        ...
        $('#searchMenu').closest('.mb-3').find('.help-block.with-errors').html('');
        $('#searchMenu').removeClass('error');

        value.searchMenuCode = $('#searchMenu').val();

        if($('#searchMenu').val() == '' || $('#searchMenu').val() == 'Select Menu'){
            validate = false;
            $('#searchMenu').addClass('error');
            $('#searchMenu').closest('.mb-3').find('.help-block.with-errors').html('<span style="color: red;">Please Select Menu</span>');
            return;
        }
