...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#confirm').on('click', function () {
		    ...
		    var contentMsg= 'Are you sure you want to edit status company to inactive ? Click "Confirm" to process or "Cancel" to cancel the submission.';
		    if ($('#inActiveFlag').val() == 'N'){
			contentMsg= 'Are you sure you want to edit status company to active? Click "Confirm" to process or "Cancel" to cancel the submission.'
	    	    }
        ...
        $('#edit').on('click', function () {
            ...
            if(res=='done'){
                ...
                $('#name').val(name);
                getDataEdit(corporate,name,countryCode,stateCode,substateCode);
            }
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
					...
					$('#inActiveReasonDetail').text(detail[index].inactiveReason);
$('#inactiveReason').text(detail[index].inactiveReason);
                    oTable_cp.clear();
                    if(contactList){
                    var cp_count = 1;
    ...
    function resetConfirm(userId){
        var content ='{{trans('form.user_msg_lock',['user'=>'$user','lock'=>'Reset'])}}';
